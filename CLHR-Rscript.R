...
# 2. Eli removed the need for GRASS with the help of Jon Clayden V 3.3

# To-do: enlarge before thinning to improve loop definition
...
drawMaps            <- F
writeResultText     <- T
allTurtleFolders    <- basename(dir('2.Turtles/', all.files = F, full.names = T))
allShoreFolders     <- basename(dir('1.Shore/', all.files = F, full.names = T))
resolution          <- '100m'
shoreFolder         <- '1.Shore'
observationFolder   <- '2.ObservedAnimalPoints'
outputDirectory     <- '3.CLHRs'
locationFilepath    <- file.path(shoreFolder,'5.Niagara-100m-output') # needs extension
locationFilename    <- paste(locationFilepath, '/','niagara100moutput.txt', sep = '')
animalNickname      <- 'cq100'
dataFilepath        <- file.path('2.Turtles','cq100output','cq100moutput') # species location info
dataFilename        <- paste(basename(dataFilepath),'.txt', sep = '')
projectionString    <- '+proj=utm +zone=17 +ellps=WGS84 +datum=WGS84 +units=m +no_defs'
# fromPrjFile         <- NULL
...
# tileGDAL <- readGDAL(locationFilename)
# image(tileGDAL, col = 'blue', axes = T, main = 'Landscape From GDAL', xlab = "Easting", ylab = "Northing")
matrixtile <- prepASCtoMatrix.fn(tile) # this function is for the niagara turtles

m <- dim(matrixtile)[1]
n <- dim(matrixtile)[2]

# Rasterize location (matrixtile) and retrieve its extents
if (drawMaps) {
  rastertile <- raster(matrixtile)
  crs(rastertile) <- projectionString
...
# Write location matrix to csv  ####
if (writeCSVs) {
  writeObservationDirectory <- file.path(outputDirectory, resolution, animalNickname, shoreFolder)
  dir.create(writeObservationDirectory, showWarnings = T, recursive = T)
  write.table(matrixtile, file = paste(writeObservationDirectory, '/', animalNickname, '-locale.csv', sep = ''), append = F, sep = ',', row.names = F, col.names = F)
}
...
# Write location skeleton matrix to csv  ####
if (writeCSVs) {
  writeSkeletonDirectory <- file.path(outputDirectory, resolution, animalNickname, '3.Skeleton')
  dir.create(writeSkeletonDirectory, showWarnings = T, recursive = T)
  write.table(rast.skeleton, file = paste(writeSkeletonDirectory, '/' ,animalNickname, '-skeleton.csv', sep = ''), append = F, sep = ',', row.names = F, col.names = F)
}
...
if (drawMaps) {
  ...
  crs(sdataRas) <- crs(rastertile)
  extent(sdataRas) <- extent(rastertile)
  plot(sdataRas, main = 'Observation Points', xlab = "Easting", ylab = "Northing", col = c('white', 'red'), legend = F)
}

# Write observations matrix to csv  ####
if (writeCSVs) {
  writeObsPointsDirectory <- file.path(outputDirectory, resolution, animalNickname, observationFolder)
  dir.create(writeObsPointsDirectory, showWarnings = T, recursive = T)
  write.table(sdata, file = paste(writeObsPointsDirectory, '/' ,animalNickname, '-ObsPointsOnly.csv', sep = ''), append = F, sep = ',', row.names = F, col.names = F)
}
...
# Write observations matrix to csv  ####
if (writeCSVs) {
  writeObsPointsOnLandDirectory <- file.path(outputDirectory, resolution, animalNickname, observationFolder)
  dir.create(writeObsPointsOnLandDirectory, showWarnings = T, recursive = T)
  write.table(map, file = paste(writeObsPointsOnLandDirectory, '/' , animalNickname, '-ObsPointsOnLandscape.csv', sep = ''), append = F, sep = ',', row.names = F, col.names = F)
}
...
if (drawMaps) {
  map2Ras <- raster(map2)
  crs(map2Ras) <- crs(rastertile)
  extent(map2Ras) <- extent(rastertile)
  plot(map2Ras, main = 'Observation Points On Skeleton', xlab = "Easting", ylab = "Northing", col = c('red', 'blue', 'green'), legend = F)
}

# Write matrix to csv  ####
if (writeCSVs) {
  writeObsPointsOnSkelDirectory <- file.path(outputDirectory, resolution, animalNickname, '4.PointsOnSkeleton')
  dir.create(writeObsPointsOnSkelDirectory, showWarnings = T, recursive = T)
  write.table(map2, file = paste(writeObsPointsOnSkelDirectory, '/' ,animalNickname, '-PointsOnSkel.csv', sep = ''), append = F, sep = ',', row.names = F, col.names = F)
}
...
if (drawMaps) {
  map3Ras <- raster(map3)
  crs(map3Ras) <- crs(rastertile)
  extent(map3Ras) <- extent(rastertile)
  plot(map3Ras, main = 'Minimum Spanning Tree', xlab = "Easting", ylab = "Northing", col = c('red', 'pink', 'blue', 'green'), legend = F)
}

# Write matrix to csv  ####
if (writeCSVs) {
  writeMSTDirectory <- file.path(outputDirectory, resolution, animalNickname, '5.MinimumSpanningTree')
  dir.create(writeMSTDirectory, showWarnings = T, recursive = T)
  write.table(map3, file = paste(writeMSTDirectory, '/' ,animalNickname, '-MST.csv', sep = ''), append = F, sep = ',', row.names = F, col.names = F)
}

# Output distance of minimum spanning tree
if (writeResultText) {
  dir.create(file.path(outputDirectory, resolution, animalNickname), showWarnings = T, recursive = T)
  write.table(mstdistance, paste(file.path(outputDirectory, resolution, animalNickname), '/',animalNickname,"-MST-dist.txt", sep = ''), sep = "\t")
}
  'The minimum spanning tree is:'; mstdistance
